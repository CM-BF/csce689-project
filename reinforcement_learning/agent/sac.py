...
def vrex_penalty(logits, labels):
    loss = F.mse_loss(logits, labels, reduction='none')
    return torch.var(loss)
...
class VRExAgent(Agent):
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, logger, step):
        ...
        self.vrex_penalty = vrex_penalty(current_Q1, target_Q) + vrex_penalty(
            current_Q2, target_Q
        )
        logger.log("train_critic/loss", critic_loss, step)
    ...
    def update(self, replay_buffer, logger, step):
        ...
        for env_id in range(self.num_envs):
            ...
            vrex_penalties.append(self.vrex_penalty)

        # Optimize the critic
        train_penalty = torch.stack(vrex_penalties).mean()
        penalty_weight = (
            self.penalty_weight if step >= self.penalty_anneal_iters else 1.0
        )
        logger.log("train_encoder/penalty", train_penalty, step)
